...
def main():
    ...
    if args.load_pretrained_model is None:
        ...
        controlnet_pretrained_path = f"out/{args.load_pretrained_model}/checkpoints/{args.load_pretrained_model_ckpt:06d}"
        controlnet = MVControlNetModel.from_unet(unet, conditioning_channels=opt.controlnet_input_channels)
        controlnet_weight_path = os.path.join(controlnet_pretrained_path, "controlnet/diffusion_pytorch_model.safetensors")
        if opt.use_peft and not os.path.exists(controlnet_weight_path):
            logger.info("No full ControlNet checkpoint found in pretrained run; initializing ControlNet weights from MVUNet.\n")
        else:
            load_checkpoint_and_dispatch(controlnet, controlnet_weight_path)

    if opt.use_peft:
        controlnet, peft_targets = peft_util.inject_lora_adapters(
            controlnet,
            opt,
            default_target_modules=["to_q", "to_k", "to_v", "to_out.0"],
        )
        logger.info(f"Enable LoRA adapters on ControlNet. Target modules: {peft_targets}\n")
        if controlnet_pretrained_path is not None:
            adapter_dir = os.path.join(controlnet_pretrained_path, "controlnet_lora")
            if peft_util.load_lora_adapter_weights(controlnet, adapter_dir, opt.peft_adapter_name):
    ...
    if opt.use_peft:
        trainable_module_names = peft_util.get_trainable_param_names(controlnet)
    else:
                ...
                if accelerator.is_main_process:
                    if opt.use_peft:
                        peft_util.save_lora_adapter(
                            ...
                            os.path.join(ckpt_dir, f"{global_update_step:06d}", "controlnet_lora"),
